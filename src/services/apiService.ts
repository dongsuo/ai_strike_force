...
export const apiService = {
  ...
  async getModels(): Promise<Model[]> {
    try {
      ...
      if (response.data.free_models && Array.isArray(response.data.free_models)) {
      ...
      return modelIds.map(modelId => {
        ...
        const modelName = nameParts.length > 1 ? 
          nameParts[1].replace(/-/g, ' ').replace(/(\d+(\.\d+)?)([a-z])/g, '$1 $3') : 
          cleanId;
        ...
        return {
          ...
          name: formattedName,
          description: `${nameParts[0] || '未知提供商'} 提供的 ${formattedName} 模型`,
          isFree: true
        };
      });
    } catch (error) {
  ...
  async startConversation(request: ConversationRequest): Promise<Conversation> {
    try {
      ...
      const initialPrompt: ChatMessage[] = [{
        role: 'system',
        content: '你是一个有用的AI助手，请直接回答用户的问题，不需要添加任何前缀。'
      }, {
        role: 'user',
        content: question
      }];
  ...
  async proceedToNextRound(conversation: Conversation): Promise<Conversation> {
    try {
      const { currentRound, maxRounds, modelIds, rounds, messages } = conversation;
      ...
      for (const modelId of modelIds) {
        ...
        // 获取模型的讨论回复
        const content = await this.createChatCompletion(modelId, discussionHistory);
        
        // 检查回复中是否包含"同意"字样，判断是否达成共识
        const hasConsensus = content.includes('同意') && !content.includes('不同意') && !content.includes('不赞同');
  ...
  // 生成最终总结
  async generateFinalSummary(conversation: Conversation): Promise<Conversation> {
    try {
      ...
      const summaryHistory: ChatMessage[] = [
        {
          role: 'system',
          content: '你是一个公正的总结者，负责整合多个AI助手的观点，提供一个全面的最终答案。'
        },
        {
          role: 'user',
      ...
      // 获取总结
      const summaryContent = await this.createChatCompletion(summarizerModelId, summaryHistory);
  ...
  // 继续现有对话 - 支持追问
  async continueConversation(request: ContinueConversationRequest, currentConversation: Conversation): Promise<Conversation> {
    try {
      const { question } = request;
      const { modelIds, models, messages, rounds, summary } = currentConversation;
      ...
      const buildChatHistoryForModel = (modelId: string) => {
        const chatHistory: ChatMessage[] = [
          {
            role: 'system',
            content: '你是一个有用的AI助手，正在回答用户的追问。请参考之前的讨论和总结。'
          }
        ];
        ...
        // 添加该模型对原始问题的回答
        const initialResponse = rounds[0].responses.find(r => r.modelId === modelId);
        if (initialResponse) {
          chatHistory.push({
            role: 'assistant',
